...
import { FolderTree, Plus, Trash2, Search, Edit2, EyeOff, Eye, Check, X, AlertCircle } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/20 backdrop-blur-sm">
      <div className="bg-white rounded-[12px] shadow-[0_24px_64px_rgba(10,17,40,0.12)] p-8 max-w-md w-full border border-border mx-4">
        <div className="flex items-center gap-4 mb-4 text-red-600">
          ...
          <h2 className="text-xl font-bold tracking-tight text-ink">Delete Category</h2>
        </div>
        
        <p className="text-ink-muted text-sm mb-6 leading-relaxed">
          Are you sure you want to delete the <span className="font-bold text-ink">"{category.name}"</span> category? 
        ...
        {hasNews && (
          <div className="bg-amber-50 border border-amber-200 rounded-[8px] p-4 mb-6">
            <div className="flex items-start gap-3 text-amber-800">
              ...
              <div>
                <p className="text-sm font-bold mb-2">This category contains {category.newsCount} news article(s).</p>
                <p className="text-xs mb-3">Please select another category to transfer them to, or choose "No Category".</p>
                <select 
                  value={transferId}
...
export default function CategoriesCmsPage() {
  ...
  const handleUpdateCategory = async (id: string, name: string, status: "active" | "hidden") => {
    try {
  ...
  const confirmDelete = async (transferToId?: string) => {
    ...
    try {
      const url = new URL(`/api/admin/news-categories/${categoryToDelete.id}`, window.location.origin);
      if (transferToId) {
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-relaxed text-ink-muted md:text-lg">
                Create, organize, and manage categories to structure your news articles and press releases.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 p-6 bg-white border border-border shadow-sm rounded-none">
          <form onSubmit={handleCreateCategory} className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <button
              ...
              className="h-12 px-8 bg-ink text-white text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-50 hover:bg-slate-800 transition-colors shrink-0"
            >
              {saving ? "Creating..." : <><Plus size={16} /> Create</>}
            </button>
          </form>
        </div>
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="border border-border bg-white px-6 py-4 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-[10px] font-bold uppercase tracking-widest text-ink-muted">Total Categories</div>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50 border-b border-border">
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-ink-muted w-1/3">Name</th>
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-ink-muted">URL Slug</th>
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-ink-muted text-center">News Posts</th>
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-ink-muted text-center">Status</th>
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-ink-muted text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-ink-muted italic">
                      Loading categories...
                    </td>
                  </tr>
                  ...
                  filteredCategories.map((cat) => (
                    <tr key={cat.id} className={`hover:bg-slate-50/50 transition-colors ${cat.status === 'hidden' ? 'bg-slate-50/30' : ''}`}>
                        ...
                        {editingId === cat.id ? (
                          <div className="flex items-center gap-2">
                            <input 
                              ...
                              autoFocus
                            />
                            <button onClick={() => handleUpdateCategory(cat.id, editingName, cat.status)} className="p-1.5 bg-accent text-white rounded hover:bg-blue-700">
                              <Check size={16} />
                            </button>
                            <button onClick={() => setEditingId(null)} className="p-1.5 bg-slate-200 text-ink rounded hover:bg-slate-300">
                              <X size={16} />
                            </button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-3">
                            <FolderTree size={16} className={cat.status === 'hidden' ? "text-slate-400" : "text-accent"} />
                            <span className={`font-bold uppercase tracking-tight text-sm ${cat.status === 'hidden' ? "text-ink-muted" : "text-ink"}`}>
                              {cat.name}
                            </span>
                          </div>
                        )}
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center justify-end gap-2">
                          <button 
                            onClick={() => handleUpdateCategory(cat.id, cat.name, cat.status === 'hidden' ? 'active' : 'hidden')}
                            className="p-2 text-ink-muted hover:text-ink hover:bg-slate-100 rounded transition-colors"
                            title={cat.status === 'hidden' ? 'Make Active' : 'Hide Category'}
                          >
                            {cat.status === 'hidden' ? <Eye size={16} /> : <EyeOff size={16} />}
                          </button>
                          <button 
                            onClick={() => {
                              setEditingId(cat.id);
